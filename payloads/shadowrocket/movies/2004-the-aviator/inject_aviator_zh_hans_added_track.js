...
const marker = "aviator-zh-hans-v1";
...
if (!target || !body.startsWith("#EXTM3U")) {
  ...
  const ordinarySpanish = media.filter(({ line }) => !/FORCED=YES/.test(line) && /URI=/.test(line) && /LANGUAGE="es(?:-[^"]+)?"/.test(line));
  if (!ordinarySpanish.length) {
    $done({});
  } else {
    const added = ordinarySpanish.map(({ line }) => {
      ...
      return `#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="${group}",LANGUAGE="zh-Hans",NAME="简体中文",AUTOSELECT=YES,DEFAULT=NO,FORCED=NO${pathwayAttribute},STABLE-RENDITION-ID="${marker}",URI="${uri}${separator}aviatorzh=1"`;
    });
