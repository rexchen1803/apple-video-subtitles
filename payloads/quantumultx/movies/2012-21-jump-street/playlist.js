...
const movieAssetId = "6792903247";
const movieSlug = "2012-21-jump-street";
const markerKey = "jumpstreetzh";
...
function rewriteOriginal(playlistBody, sourceUrl) {
  ...
  const resource = "P1464582090_A6792903247";
  const url = __originalRequest.url || "";
  const body = __originalResponse.body || "";
  const marked = /[?&]jumpstreetzh=1(?:&|$)/.test(url);
  ...
  if (!marked || !target || !body.startsWith("#EXTM3U")) {
    ...
    if (leaves.length !== 1 || lines.some((line) => /(?:pre|post|empty)/i.test(line) && line.includes(resource))) {
      ...
      const rewritten = lines.map((line) => {
        if (!line.includes(resource) || !/_en_subtitles_V2-[^\s?]*\.webvtt/.test(line) || /[?&]jumpstreetzh=1(?:&|$)/.test(line)) return line;
        return `${line}${line.includes("?") ? "&" : "?"}jumpstreetzh=1`;
      }).join(body.includes("\r\n") ? "\r\n" : "\n");
