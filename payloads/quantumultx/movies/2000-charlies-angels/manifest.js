...
const movieAssetId = "6793717567";
const movieSlug = "2000-charlies-angels";
const markerKey = "charliesangelszh";
...
function injectOriginal(masterBody, sourceUrl) {
  ...
  const assetId = "6793717567";
  const marker = "charliesangelszh=1";
  const url = __originalRequest.url || "";
  const body = __originalResponse.body || "";
  const target = new RegExp("(?:[?&](?:mainAssetAdamId|a)=[^&#]*6793717567(?:[&#]|$)|_A6793717567_)").test(url);
  if (!target || !body.startsWith("#EXTM3U")) { __captureDone({}); }
  else if (body.includes('STABLE-RENDITION-ID="charlies-angels-zh-hans-v1"')) { __captureDone({ body }); }
  else {
    ...
    if (!english.length) __captureDone({});
    else {
      const added = english.map(({ line }) => {
        ...
        return `#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="${group}",LANGUAGE="zh-Hans",NAME="简体中文",AUTOSELECT=YES,DEFAULT=NO,FORCED=NO${pathwayAttribute},STABLE-RENDITION-ID="charlies-angels-zh-hans-v1",URI="${uri}${separator}charliesangelszh=1"`;
      });
      if (added.some((value) => !value)) __captureDone({});
      else { const insertAt = media.length ? media[0].index : lines.length; lines.splice(insertAt, 0, ...added); __captureDone({ body: lines.join(eol) }); }
    }
  }
