...
const movieAssetId = "6793692535";
const movieSlug = "2007-zodiac";
const markerKey = "zodiaczh";
...
function injectOriginal(masterBody, sourceUrl) {
  ...
  const marker = "zodiac-zh-hans-v1";
  ...
  if (!target || !body.startsWith("#EXTM3U")) {
    ...
    if (!spanish.length || (pathways.length > 0 && new Set(pathways).size !== pathways.length)) {
      ...
      const added = spanish.map(({ line }) => {
        ...
        return `#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="${group}",LANGUAGE="zh-Hans",NAME="简体中文",AUTOSELECT=YES,DEFAULT=NO,FORCED=NO${pathwayAttribute},STABLE-RENDITION-ID="${marker}",URI="${uri}${separator}zodiaczh=1"`;
      });
