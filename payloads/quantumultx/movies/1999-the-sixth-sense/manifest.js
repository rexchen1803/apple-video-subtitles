...
const movieAssetId = "6793615720";
const movieSlug = "1999-the-sixth-sense";
const markerKey = "sixthsensezh";
...
function injectOriginal(masterBody, sourceUrl) {
  ...
  const marker = "sixthsense-zh-hans-v1";
  ...
  if (!target || !body.startsWith("#EXTM3U")) {
    ...
    if (!english.length || (pathways.length > 0 && new Set(pathways).size !== pathways.length)) {
      ...
      const added = english.map(({ line }) => {
        ...
        return `#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="${group}",LANGUAGE="zh-Hans",NAME="简体中文",AUTOSELECT=YES,DEFAULT=NO,FORCED=NO${pathwayAttribute},STABLE-RENDITION-ID="${marker}",URI="${uri}${separator}sixthsensezh=1"`;
      });
