...
const movieAssetId = "6793683930";
const movieSlug = "2012-looper";
const markerKey = "looperzh";
...
function injectOriginal(masterBody, sourceUrl) {
  ...
  const marker = "looper-zh-hans-v1";
  ...
  if (!target || !body.startsWith("#EXTM3U")) {
    ...
    const forcedEnglish = media.filter(({ line }) => /FORCED=YES/.test(line) && /LANGUAGE="en(?:-[^"]+)?"/.test(line) && /URI=/.test(line));
    const donors = english.length ? english : forcedEnglish;
    const pathways = donors.map(({ line }) => line.match(/PATHWAY-ID="([^"]+)"/)?.[1]).filter(Boolean);
    if (!donors.length || (pathways.length > 0 && new Set(pathways).size !== pathways.length)) {
      __captureDone({});
    } else {
      const added = donors.map(({ line }) => {
        ...
        return `#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="${group}",LANGUAGE="zh-Hans",NAME="简体中文",AUTOSELECT=YES,DEFAULT=NO,FORCED=NO${pathwayAttribute},STABLE-RENDITION-ID="${marker}",URI="${uri}${separator}looperzh=1"`;
      });
