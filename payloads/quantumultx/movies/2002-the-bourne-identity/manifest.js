...
const movieAssetId = "6800163537";
const movieSlug = "2002-the-bourne-identity";
const markerKey = "bournezh";
...
function injectOriginal(masterBody, sourceUrl) {
  ...
  const marker = "bourne-zh-hans-v1";
  ...
  if (!target || !body.startsWith("#EXTM3U")) {
    ...
    const donors = spanish;
    const pathways = donors.map(({ line }) => line.match(/PATHWAY-ID="([^"]+)"/)?.[1]).filter(Boolean);
    if (!donors.length || new Set(pathways).size !== pathways.length) {
      __captureDone({});
    } else {
      const added = donors.map(({ line }) => {
        ...
        return `#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="${group}",LANGUAGE="zh-Hans",NAME="简体中文",AUTOSELECT=YES,DEFAULT=NO,FORCED=NO${pathwayAttribute},STABLE-RENDITION-ID="${marker}",URI="${uri}${separator}bournezh=1"`;
      });
