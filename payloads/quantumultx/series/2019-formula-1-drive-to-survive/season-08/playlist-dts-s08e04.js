...
const assetId = "1880613354"; const slug = "dts-s08e04";
...
if (!/^https:\/\/play(?:-edge)?\.itunes\.apple\.com\//i.test(sourceUrl) || !sourceUrl.includes(assetId)) fail(400, "Invalid series playlist source");
  ...
  __qxDone({ response: { status: 200, headers: { "Content-Type": "application/vnd.apple.mpegurl", "Cache-Control": "no-store, max-age=0" }, body } });
});
