...
function bridgeHlsUrl(value) {
  ...
  for (const assetId of Object.keys(targets)) {
    ...
    return bridgeBase + targets[assetId] + "/index.m3u8?src=" + encodeURIComponent(source);
  }
  return value;
}
...
function rewrite(value) {
  ...
  for (const key of Object.keys(value)) {
    output[key] = key === "hlsUrl" ? bridgeHlsUrl(value[key]) : rewrite(value[key]);
  }
  return output;
}
