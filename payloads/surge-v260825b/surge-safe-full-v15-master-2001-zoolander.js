...
const movieAssetId = "6793692948";
const movieSlug = "2001-zoolander";
const markerKey = "zoolanderzh";
...
function injectOriginal(masterBody, sourceUrl) {
  ...
  if (!target || !body.startsWith("#EXTM3U")) { __captureDone({}); }
  else if (body.includes('STABLE-RENDITION-ID="zoolander-zh-hans-v1"')) { __captureDone({ body }); }
  else {
    ...
    if (!spanish.length) __captureDone({});
    else {
