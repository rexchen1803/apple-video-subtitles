...
const movieAssetId = "6793304264";
const movieSlug = "2004-i-robot";
const markerKey = "irobotzh";
...
function injectOriginal(masterBody, sourceUrl) {
  ...
  const marker = "irobot-zh-hans-v1";
  ...
  if (!target || !body.startsWith("#EXTM3U")) {
    ...
    const english = ordinary.filter(({ line }) => /LANGUAGE="en(?:-[^"]+)?"/.test(line) && !/LANGUAGE="zh-Hans"/.test(line));
    if (!english.length) {
      __captureDone({});
    } else {
      const added = english.map(({ line }) => {
        ...
        return `#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="${group}",LANGUAGE="zh-Hans",NAME="简体中文",AUTOSELECT=YES,DEFAULT=NO,FORCED=NO${pathwayAttribute},STABLE-RENDITION-ID="${marker}",URI="${uri}${separator}irobotzh=1"`;
      });
