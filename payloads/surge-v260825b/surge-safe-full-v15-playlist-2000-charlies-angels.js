...
function rewriteOriginal(playlistBody, sourceUrl) {
  ...
  const resource = "P1466465179_A6793717567";
  const url = __originalRequest.url || "";
  const body = __originalResponse.body || "";
  const marked = /[?&]charliesangelszh=1(?:&|$)/.test(url);
  const target = new RegExp("(?:[?&](?:mainAssetAdamId|a)=6793717567(?:[&#]|$)|_A6793717567_)").test(url);
  if (!marked || !target || !body.startsWith("#EXTM3U")) { __captureDone({}); }
  else {
    const lines = body.split(/\r?\n/);
    const leaves = lines.filter((line) => line.includes(resource) && /_en_subtitles_V2-[^\s?]*\.webvtt/.test(line));
    if (leaves.length !== 1 || lines.some((line) => /(?:pre|post|empty)/i.test(line) && line.includes(resource))) { __captureDone({}); }
    else { const rewritten = lines.map((line) => { if (!line.includes(resource) || !/_en_subtitles_V2-[^\s?]*\.webvtt/.test(line) || /[?&]charliesangelszh=1(?:&|$)/.test(line)) return line; return `${line}${line.includes("?") ? "&" : "?"}charliesangelszh=1`; }).join(body.includes("\r\n") ? "\r\n" : "\n"); __captureDone({ body: rewritten }); }
  }
