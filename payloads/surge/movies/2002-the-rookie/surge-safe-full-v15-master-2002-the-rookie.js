...
const movieAssetId = "6793717467";
const movieSlug = "2002-the-rookie";
const markerKey = "rookiezh";
...
function injectOriginal(masterBody, sourceUrl) {
  ...
  const assetId = "6793717467";
  const marker = "rookie-zh-hans-v1";
  ...
  if (!target || !body.startsWith("#EXTM3U")) {
    ...
    if (!english.length) {
      ...
      const added = english.map(({ line }) => {
        ...
        return `#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="${group}",LANGUAGE="zh-Hans",NAME="简体中文",AUTOSELECT=YES,DEFAULT=NO,FORCED=NO${pathwayAttribute},STABLE-RENDITION-ID="${marker}",URI="${uri}${separator}rookiezh=1"`;
      });
